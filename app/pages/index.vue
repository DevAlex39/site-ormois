...
<template>
  <div class="home">
    ...
    <section id="sec-accueil" data-section="accueil" class="hero">
      <img
        src="/Accueil.jpg"
        alt="Serre de la Ferme de l'Ormois"
        class="hero-bg"
      />
      ...
      <div class="hero-content">
        <p class="hero-kicker">
          {{
            c.general_slogan ||
            "Maraîchage en circuit court · Marandeuil, Côte-d'Or"
          }}
        </p>
        <h1 class="hero-title">La Ferme<br />de l'Ormois</h1>
        <p class="hero-sub">
          Des légumes frais et de saison, cultivés avec soin et récoltés au bon
          moment.
        </p>
        <div class="hero-actions">
          <button class="btn btn-cream" @click="scrollTo('produits')">
            Découvrir nos produits
          </button>
          <NuxtLink to="/commander" class="btn btn-ghost"
            >Passer commande</NuxtLink
          >
          <a
            href="https://www.facebook.com/lafermedelormois"
            target="_blank"
            rel="noopener"
            class="btn btn-fb"
          >
      ...
      <div class="ferme-grid">
        <div>
          ...
          <h2 class="section-title">
            {{
              c.ferme_titre ||
              "Une agriculture biologique, au rythme des saisons"
            }}
          </h2>
          ...
          <NuxtLink to="/contact" class="btn btn-outline"
            >Nous rendre visite</NuxtLink
          >
        </div>
        <div class="ferme-media">
          <div class="ferme-photo">
            <img
              :src="c.ferme_image || '/Serre.jpg'"
              alt="Serre de tomates en pleine culture"
            />
          </div>
    ...
    <section id="sec-apropos" data-section="apropos" class="apropos">
      <div class="apropos-grid">
        <div class="apropos-media">
          <img
            :src="c.apropos_image || '/Serre.jpg'"
            alt="La Ferme de l'Ormois"
          />
          <div class="apropos-badge">
            <span class="badge-line1">{{
              c.apropos_badge1 || "Du semis"
            }}</span>
            <span class="badge-line2">{{
              c.apropos_badge2 || "jusqu'à votre panier"
            }}</span>
          </div>
        </div>
        <div class="apropos-body">
          ...
          <h2 class="section-title">
            {{ c.apropos_titre || "L'histoire de la Ferme de l'Ormois" }}
          </h2>
          ...
          <p class="apropos-signature" v-if="c.apropos_signature">
            — <em>{{ c.apropos_signature }}</em>
          </p>
        </div>
      </div>
    </section>
      ...
      <div class="inner">
        <div class="center-head">
          ...
          <p class="section-subtitle">
            Disponibles selon la saison et les récoltes
          </p>
        </div>
        ...
        <div class="products-grid" v-if="filtered.length">
          ...
          >
            ...
            <div class="prod-body">
              ...
              <div class="prod-meta">
                <span class="prod-price" v-if="p.prix"
                  >{{ formatPrice(p.prix) }} € / {{ p.unite }}</span
                >
    ...
    <section id="sec-galerie" data-section="galerie" class="galerie">
      <div class="inner">
        <div class="center-head">
          ...
          <h2 class="section-title" style="color: var(--beige)">
    ...
    <section id="sec-contact" data-section="contact" class="contact">
      <div class="contact-grid">
        <div>
            ...
            <div class="info-block">
              ...
              <div>
                ...
                ><a
                  :href="
                    'tel:+33' +
                    (c.contact_telephone || '0620499321')
                      .replace(/^0/, '')
                      .replace(/\s/g, '')
                  "
                  >{{ c.contact_telephone || "06 20 49 93 21" }}</a
                >
              </div>
            </div>
            <div class="info-block">
              ...
              <div>
                ...
                ><a
                  :href="
                    'mailto:' +
                    (c.contact_email || 'lafermedelormois@gmail.com')
                  "
                  >{{ c.contact_email || "lafermedelormois@gmail.com" }}</a
                >
              </div>
            </div>
          ...
          <div class="contact-actions">
            <NuxtLink to="/commander" class="btn btn-accent"
              >Passer commande</NuxtLink
            >
            <a
              :href="
                c.contact_facebook ||
                'https://www.facebook.com/lafermedelormois'
              "
              target="_blank"
              rel="noopener"
              class="btn btn-fb"
      ...
      <footer class="home-footer">
        ...
        <span class="home-footer-copy"
          >© {{ year }} La Ferme de l'Ormois — Maraîchage en circuit court,
          Marandeuil</span
        >
      </footer>
    </section>
    ...
    >
      ...
      <img
        :src="galleryPhotos[lightbox!]?.path"
...
const { data: contenu } =
  await useFetch<Record<string, string>>("/api/contenu");
const { data: galerieItems } =
  await useFetch<{ id: number; path: string }[]>("/api/galerie");
...
function fallbackImg(i: number) {
  return productFallbacks[i % productFallbacks.length];
}
...
function prevPhoto() {
  lightbox.value =
    (lightbox.value! - 1 + galleryPhotos.value.length) %
    galleryPhotos.value.length;
}
function nextPhoto() {
  lightbox.value = (lightbox.value! + 1) % galleryPhotos.value.length;
}
...
function scrollTo(id: string) {
  ...
  if (!el) return;
  const top =
    id === "accueil" ? 0 : el.getBoundingClientRect().top + window.scrollY;
...
.hero-overlay {
  ...
  background: linear-gradient(
    180deg,
    rgba(24, 46, 26, 0.62) 0%,
    rgba(26, 46, 26, 0.46) 45%,
    rgba(30, 38, 20, 0.68) 100%
  );
}
.hero-content {
  position: relative;
...
.apropos-body .eyebrow {
  margin-bottom: 0.8rem;
}
.apropos-body .section-title {
...
.prod-cat {
  ...
  background: rgba(47, 107, 57, 0.92);
  color: #fff;
...
.info-icon {
  ...
  height: 42px;
  border-radius: 50%;
  background: rgba(232, 160, 106, 0.18);
  color: var(--orange);
  display: inline-flex;
  align-items: center;
  justify-content: center;
