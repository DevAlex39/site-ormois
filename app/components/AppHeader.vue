    ...
    <div class="container header-inner">
      ...
      <a href="/" class="logo" @click="onLogo">
        <span class="logo-mark">
          <img
            v-if="contenu?.general_logo"
            :src="contenu.general_logo"
            alt="Logo"
            class="logo-img"
          />
        ...
        <span class="logo-text">
          ...
          <strong>{{
            contenu?.general_nom?.replace(/^La Ferme de /i, "") || "L'ORMOIS"
          }}</strong>
        </span>
      </a>
...
const { data: contenu } = await useFetch<Record<string, string>>(
  "/api/contenu",
  { key: "site-contenu" },
);
...
const solid = computed(
  () => !isHome.value || scrolled.value || active.value !== "accueil",
);
...
function go(id: string) {
  ...
  if (isHome.value) {
    ...
    router
      .push("/")
      .then(() => nextTick(() => setTimeout(() => scrollToSection(id), 120)));
  }
}
...
function setupObserver() {
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((e) => {
        if (e.isIntersecting)
          active.value = (e.target as HTMLElement).dataset.section || "accueil";
      });
    },
...
watch(
  () => route.path,
  () => nextTick(() => setTimeout(setupObserver, 100)),
);
</script>
